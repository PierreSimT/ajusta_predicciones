...
class Model(ABC):
    ...
    def train(self, X_train, X_test, y_train, y_test, output_filename, n_epochs):
        pass
    ...
    def get_statistics(self, X_test, y_test, time, output_directory):
        ...
        result = result.append({'Model': self.name, 'Variable': str(y_test.columns[0]), 'MAE': mae, 'MBE': mbe, 'MBE': mbe, 'MSE': mse, 'MSRE': msre, 'EVS': evs, 'CORR': corr, 'STD': std, 'Time': time}, ignore_index=True)
